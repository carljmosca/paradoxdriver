...
public class ParadoxStatement implements Statement {
    ...
    private void executeSelect(final SelectNode node) throws SQLException {
        ...
        columns.add(new Column("first_name", Types.VARCHAR));
        final ArrayList<ArrayList<FieldValue>> values = new ArrayList<ArrayList<FieldValue>>();
        values.add(new ArrayList<FieldValue>());
        values.get(0).add(new FieldValue("Philip", Types.VARCHAR));
        
        for (SQLNode sqlNode : node.getFields()) {
            System.out.println("Field Name: " +  sqlNode.getName());
            
            if ("*".equals(sqlNode.getName())) {
                for (TableNode tableNode : node.getTables()) {
                    System.out.println("Table Name: " + tableNode.getName());
                }
            } else {
                //columns.add(new Column(sqlNode.getName()));
            }
        }        
