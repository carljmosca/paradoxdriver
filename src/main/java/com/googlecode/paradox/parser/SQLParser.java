...
public class SQLParser {
    ...
    private final String TABLE_PREFIX = "\"APP\".";

    public SQLParser(final String sql) throws SQLException {
        String s = sql;
        if (sql.toUpperCase().contains(TABLE_PREFIX)) {
            s = sql.toUpperCase().replace(TABLE_PREFIX, "");
        }
        this.sql = s;
        scanner = new Scanner(this.sql);
    }
     ...
     *
     * @param parent Statement Owner
     * @return a Select Statement Node
     * @throws SQLException Invalid SQL
     * @throws IOException in case of parser exception
     */
